...
class VillageScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.nearExit = false;
    this.autoTriggeredId = null; // 記錄目前已自動觸發過的建築，離開範圍才會重置，避免重複彈出
  ...
  update(time) {
    ...
    if (closest && closest.buildingData.type === "quiz") {
      ...
      // 走到建築物定點即自動觸發故事，不需要再按互動鍵；離開範圍後才能再次觸發同一棟建築
    ...
    // 離開所有建築物範圍時重置自動觸發記錄，下次走回去會再次觸發故事選單
    if (!closest) {
...
function resumeVillageScene() {
  if (game && game.scene.isPaused("VillageScene")) {
...
function closeQuiz() {
  ...
  resumeVillageScene();
  // 注意：故意不在此處 restart 場景，避免玩家仍站在建築物範圍內時，自動觸發機制重新偵測而立刻再次彈出同一個關卡選單。
  // 建築物上方「已完成數/總數」的數字，會在玩家下次重新進入這座村莊（例如先回鎮口再進來）時自動更新為最新進度。
}
