...
class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    // 中央天秤
    this.add.image(w/2, h-110, "b_scale").setScale(1.1).setDepth(6);
    ...
    // 主標題
    this.add.text(w/2, h-170, "🏯 刑法鎮 LawTown", { fontSize: "18px", color: "#fff8e1", fontStyle: "bold", backgroundColor: "#00000055", padding: { x: 10, y: 5 } }).setOrigin(0.5).setDepth(9);
    this.add.text(w/2, h-148, "⚖ 法治廣場", { fontSize: "12px", color: "#e8c873" }).setOrigin(0.5).setDepth(9);
 
    // 互動按鈕（Phaser原生按鈕，不用HTML覆蓋層）
...
class VillageScene extends Phaser.Scene {
  ...
  createTouchControls() {
    ...
    const up = mkBtn(baseX, baseY-50, "↑"), down = mkBtn(baseX, baseY+10, "↓");
    const left = mkBtn(baseX-50, baseY-20, "←"), right = mkBtn(baseX+50, baseY-20, "→");
    ...
    [["up",up],["down",down],["left",left],["right",right]].forEach(([k,b]) => {
      b.on("pointerdown", () => this.touchState[k]=true);
      b.on("pointerup", () => this.touchState[k]=false);
      b.on("pointerout", () => this.touchState[k]=false);
    });
